                  ...
                  char *msg)
{
    ...
    for (int s_i = 0; s_i < n; s_i++)
    {
        ...
        double dion, dtrp, vmeas, vion, vtrp, rr[3], pos[3], e[3], P, lam_L1, freq;
        ...
        // 电离层延迟
        if (!ionocorr(obs[s_i].time, nav, obs[s_i].sat, pos, azel_ + s_i * 2, opt_.ionoopt, &dion, &vion))
        {
            continue;
        }
        ...
        // 对流层延迟
        if (!tropcorr(obs[s_i].time, nav, pos, azel_ + s_i * 2, opt_.tropopt, &dtrp, &vtrp))
        {
            continue;
        }
        ...
        // gnss_raw.sat_clk_drift = dts[1 + s_i * 2] * CLIGHT;
        ...
        gnss_raw.lamda = CLIGHT / freq; // 这里求波长的方法有些许变化

        int sys = satsys(obs[s_i].sat, NULL);
        // gnss_raw.receiver_clk_err = sol->dtr[0] * CLIGHT;
        // gnss_raw.freq = freq;
        // gnss_raw.doppler = obs[s_i].D[0];
        // gnss_raw.sat_vel_x = rs[3 + s_i * 6];
        // gnss_raw.sat_vel_y = rs[4 + s_i * 6];
        // gnss_raw.sat_vel_z = rs[5 + s_i * 6];
