                  ...
                  const nav_t *nav)
{
    ...
    std::cout << "用户和基站的时间差-> "<< dt << " 用户数量-> " << nu << " 基站数量-> " << nr <<std::endl;
    ...
    int current_week;
    double current_tow;
    // FILE* fp = fopen("/home/liziyao/GraphGNSSLib_libRSF/GraphGNSSLib/src/result/station_raw.txt", "a");
    // current_tow = time2gpst(obs[0].time, &current_week);
    // std::cout << "current_tow-> "<< current_tow <<std::endl;
    // fprintf(fp, "用户卫星current_tow-> %f    ", current_tow);
    current_tow = time2gpst(obs[nu].time, &current_week);
    // std::cout << "current_tow-> "<< current_tow <<std::endl;
    // fprintf(fp, "基站卫星current_tow-> %f\n", current_tow);
    // fclose(fp);
    for(int is=0; is<ns; is++)
    {
        ...
        // if(is>=nu)
        {
            ...
            double temp_freq = sat2freq(obs[ir[is]].sat, obs[ir[is]].code[0], nav);
            gnss_raw.raw_pseudorange = obs[ir[is]].P[0];
            gnss_raw.carrier_phase = obs[ir[is]].L[0];
            gnss_raw.lamda = CLIGHT / temp_freq;
            ...
            double sat_pos[3] = {gnss_raw.sat_pos_x, gnss_raw.sat_pos_y,gnss_raw.sat_pos_z};
            double rr[3] = {station_x,station_y,station_z};
            double e[3]={0};
            double r = geodist(sat_pos,rr,e);
            double pos[3] = {0};
            double azel[3] = {0};
            ecef2pos(rr,pos);
            satazel(pos, e,azel);
            ...
            // gnss_raw.freq = temp_freq;
            // gnss_raw.doppler = obs[ir[is]].D[0];
            // gnss_raw.sat_vel_x = rs[3 + ir[is] * 6];
            // gnss_raw.sat_vel_y = rs[4 + ir[is] * 6];
            // gnss_raw.sat_vel_z = rs[5 + ir[is] * 6];
            // gnss_raw.psr_weight = -1;
            // gnss_raw.doppler_weight = -1;
            // gnss_raw.cp_weight = -1;
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    // FILE* fp_stat = fopen("/home/liziyao/GraphGNSSLib_libRSF/GraphGNSSLib/src/result/time_check.txt", "a");
    // current_tow = time2gpst(obs[0].time, &current_week);
    // fprintf(fp_stat, "用户卫星current_tow-> %f    ", current_tow);
    // current_tow = time2gpst(obs[nu].time, &current_week);
    // fprintf(fp_stat, "基站卫星current_tow-> %f\n", current_tow);
    // fclose(fp_stat);
